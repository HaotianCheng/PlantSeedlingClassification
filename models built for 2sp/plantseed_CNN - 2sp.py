...
# directories
cw_dir = os.getcwd()
data_dir = 'C:\\Galaxy\\Tools\\Scripts\\PlantSeedling\\cnn'
train_dir = os.path.join(data_dir, 'train')
test_dir = os.path.join(data_dir, 'test')

# different species in the data set
species = ['Black-grass', 'Loose Silky-bent']
...
if take_train_samples:
    train_df = pd.concat([train_df[train_df['species'] == sp][:250] for sp in species])
...
if not pre_train:
    for j in range(cv_num):
        ...
        # saver.restore(sess, "C:\\Galaxy\\Tools\\XM\\htdocs\\plantseedling\\my_model\\models2.ckpt")
        ...
        saver.save(sess, 'C:\\Galaxy\\Tools\\XM\\htdocs\\plantseedling\\my_model\\models5.ckpt')
        sess.close()
else:
    with tf.Session() as sess:
        saver.restore(sess, "C:\\Galaxy\\Tools\\XM\\htdocs\\plantseedling\\my_model\\models5.ckpt")
        y_test_pred_proba_DNN += y_pred.eval(feed_dict={x_data: x_test_bf, tf_keep_prob: 1.0})
